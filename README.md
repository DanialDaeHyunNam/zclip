...
## The hosted site is an *about page* — the studio runs on **your** machine

ZCLIP is deliberately **local-first**. The in-app key writer and the GRAB
tool only run in dev mode, and every take spends real money against whatever
keys are configured — so a public URL with a working studio would spend the
host's keys for anyone who finds it. Because of that:

- **The public Vercel deployment is a marketing / about page.** The landing
  works; opening the studio (`/chat`) there shows a **local-install guide**
  (macOS / Windows, English / 한국어) instead of the studio — see
  [`/install`](app/install/page.tsx).
- **Running it locally (`bun dev`) gives you the real studio**, no gate.
- Cloud-vs-local is decided in one place, [`lib/deploy.ts`](lib/deploy.ts)
  (`isCloud()` = `process.env.VERCEL === "1"`). Set `ZCLIP_CLOUD=1` to preview
  the gated experience locally, or `ZCLIP_CLOUD=0` to unlock the studio on a
  deployment **you trust and password-protect** (see [Deploying](#deploying-to-vercel)).
...
- `app/chat/page.tsx` — server gate: the real studio locally, the
  local-install guide on the cloud deploy. The studio UI itself is
...
A default deploy is an **about page**: the landing renders, and the studio
route serves the local-install guide (see
[the hosted-site note above](#the-hosted-site-is-an-about-page--the-studio-runs-on-your-machine)).
That's the safe default — no keys required, nothing to spend.

```bash
vercel                                    # first deploy, accept defaults
vercel --prod                             # → landing + /install guide, no studio
```

**Want a working *hosted* studio for a trusted team?** Unlock the gate and
lock the door at the same time:

```bash
vercel env add APP_PASSWORD production     # REQUIRED — shared password on every API route
vercel env add ZCLIP_CLOUD production      # value: 0  → unlock the studio on this deploy
vercel env add GEMINI_API_KEY production   # + any other provider keys you wire
vercel --prod
```

Without `APP_PASSWORD`, an unlocked hosted studio spends **your** keys for
anyone who finds the URL. GRAB and the in-UI key writer stay disabled in
production regardless (they shell out / write files — dev only); on a hosted
studio, set provider keys as Vercel env vars instead of pasting them in the UI.
