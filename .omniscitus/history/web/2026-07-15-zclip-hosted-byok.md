...
- [x] Vercel Web Analytics + trackEvent no-op 래퍼 — 2026-07-21 배선
      (@vercel/analytics@2.0.1). `app/analytics.tsx`(ZAnalytics, "use
      client", beforeSend로 쿼리스트링 제거 → 경로만), layout에서
      `isCloud() && <ZAnalytics/>`로 cloud 전용 마운트(로컬/셀프호스트
      호출 0), `lib/analytics.ts` track()은 data-hosted="1" 게이트 no-op
      래퍼(coarse non-PII props만, 프롬프트/키/URL 금지). tsc 통과.
      ⚠️ 실수집엔 Vercel 대시보드 Analytics 탭 Enable + 재배포 필요
- [ ] /privacy 페이지 (2트랙 계획 잔여 — 쿠키리스/무PII라 법적 필수는
      아니나 계획 항목; memory: two-track-distribution-plan)
